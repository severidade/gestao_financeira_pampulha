function processarRelatorioBackend(chaveData) {
  ...
  const MARGEM_ERRO = 0.05; // Aceita diferença de até 5 centavos (arredondamento)

  const [mesAlvo, anoAlvo] = chaveSelecionada.split("|");

  // --- FUNÇÃO AUXILIAR PARA LIMPAR NÚMEROS (Importante para comparar) ---
  const converterParaFloat = (v) => {
    ...
    return (
      parseFloat(
        String(v).replace("R$", "").replace(/\./g, "").replace(",", ".").trim(),
      ) || 0
    );
  };
  ...
  // --- 2. BUSCAR DADOS DO ACERTO (Valor que você lançou para cobrar) ---
  ...
  for (let i = 1; i < dadosAcertos.length; i++) {
    if (
      String(dadosAcertos[i][0]) === mesAlvo &&
      String(dadosAcertos[i][1]) === anoAlvo
    ) {
      ...
      dadosCobranca = {
        ...
        valor: dadosAcertos[i][3], // Valor vindo da planilha
  ...
  if (!dadosCobranca)
    throw new Error("Cobrança não encontrada na tabela de Acertos.");

  // Prepara valor cobrado para cálculo matemático
  let valorCobradoNumerico = converterParaFloat(dadosCobranca.valor);

  // Formata para exibição no HTML
  let valorFinalDisplay = valorCobradoNumerico.toLocaleString("pt-BR", {
    style: "currency",
    currency: "BRL",
  });

  // --- 3. BUSCAR PASSIVOS E CALCULAR TOTAL ---
  const dadosPassivos = abaPassivos.getDataRange().getValues();
  const ricosPassivos = abaPassivos.getDataRange().getRichTextValues();
  let htmlLista = "";
  let anexos = [];

  let totalPassivosAcumulado = 0; // <--- NOVA VARIÁVEL PARA SOMA
  ...
  for (let i = 1; i < dadosPassivos.length; i++) {
    ...
    if (passivoMes === mesAlvo && passivoAno === anoAlvo) {
      ...
      // Converte para número para somar
      let valPassivoNum = converterParaFloat(valPassivoRaw);
      totalPassivosAcumulado += valPassivoNum; // <--- SOMA AQUI

      // Formata para lista
      let valPassivoFmt = valPassivoNum.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
      });
      ...
      if (!url && String(dadosPassivos[i][4]).includes("http"))
        url = dadosPassivos[i][4];

      if (url) {
        try {
          let idArquivo = "";
          if (url.includes("id="))
            idArquivo = url.split("id=")[1].split("&")[0];
          else if (url.includes("/d/"))
            idArquivo = url.split("/d/")[1].split("/")[0];
          if (idArquivo) {
            let blob = DriveApp.getFileById(idArquivo)
              .getBlob()
              .setName(`${servico}.pdf`);
            anexos.push(blob);
          }
        } catch (e) {
          console.log("Erro anexo: " + e.message);
        }
      }
    }
  }
  ...
  // --- 4. 🚨 VERIFICAÇÃO DE SEGURANÇA (O ALERTA) ---
  let valorIndividualCalculado = totalPassivosAcumulado / NUMERO_PESSOAS;
  let diferenca = Math.abs(valorCobradoNumerico - valorIndividualCalculado);

  if (diferenca > MARGEM_ERRO) {
    let calculadoFmt = valorIndividualCalculado.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL",
    });

    throw new Error(
      `⚠️ DIVERGÊNCIA DE VALORES!\n\n` +
        `Valor que você está cobrando: ${valorFinalDisplay}\n` +
        `Soma das despesas dividida por 3: ${calculadoFmt}\n\n` +
        `Diferença: R$ ${diferenca.toFixed(2)}\n` +
        `Verifique se lançou todas as contas na aba Passivos ou se o valor do Acerto está correto.`,
    );
  }
  ...
  let qrCodeBlob = null;
  try {
    let qrUrl = dadosCobranca.qrRichText
      ? dadosCobranca.qrRichText.getLinkUrl()
      : null;
    if (!qrUrl && String(dadosCobranca.qrTextoBackup).includes("http"))
      qrUrl = dadosCobranca.qrTextoBackup;

    if (qrUrl) {
      let idQr = "";
      if (qrUrl.includes("id=")) idQr = qrUrl.split("id=")[1].split("&")[0];
      else if (qrUrl.includes("/d/"))
        idQr = qrUrl.split("/d/")[1].split("/")[0];
      if (idQr)
            ...
            <tr>
              <td style="padding:16px 24px;">
                <div style="background:#e8f0fe; padding:20px; border-radius:8px; text-align:center; border: 1px solid #d2e3fc;">
                  <div style="font-size:14px; color:#1155cc; margin-bottom:10px; font-weight:bold; text-transform:uppercase;">Vencimento: ${dadosCobranca.vencimento}</div>
                  <div style="font-size:28px; font-weight:bold; color:#000;">${valorFinalDisplay}</div>
                  <div style="font-size:12px; color:#666;">(Valor da cota individual)</div>
                </div>
              </td>
            </tr>
            <tr>
